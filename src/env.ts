...
export function loadEnv(): Env {
  ...
  const upstashRedisUrl = optional("UPSTASH_REDIS_URL");
  const upstashRedisToken = optional("UPSTASH_REDIS_TOKEN");
  const redisUrl =
    optional("REDIS_URL") ??
    upstashRedisUrl ??
    (upstashRestUrl && upstashRestToken
      ...
      : undefined);
  if (!redisUrl) {
    throw new Error("Missing env var: REDIS_URL (or UPSTASH_REDIS_URL/UPSTASH_REDIS_REST_URL)");
  }
